...
export default function SignalMatrix({ stages, onToggleSignal }) {
  ...
  const categories = [...new Set(signals.map(c => c.category))]
    ...
    <div className="flex flex-col gap-12">
            ...
            {categorySignals.map(signal => {
              ...
              return (
                ...
                >
                  ...
                  {stages.map(stage => {
                    const isActive = (stage.signals || []).includes(signal.id)
                    return (
      ...
      {(() => {
        const uncovered = signals.filter(
          c => !stages.some(s => (s.signals || []).includes(c.id))
        )
        if (uncovered.length === 0 || uncovered.length === signals.length) return null
        return (
